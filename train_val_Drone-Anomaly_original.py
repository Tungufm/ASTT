...
def train_epoch(epoch, model, data_loader, criterion, optimizer, lr_scheduler, metrics, device=torch.device('cpu')):
    ...
    for batch_idx, (batch_data) in enumerate(data_loader):
        batch_data_256, batch_data = batch_data['standard'].to(device), batch_data['standard'].to(device)
        optimizer.zero_grad()
...
def valid_epoch(epoch, model, data_loader, criterion, metrics, device=torch.device('cpu')):
    ...
    acc5s = []
    # validation loop
    new_label = np.load("../../Crossroads/test/test_frame_masks/01.npy")
...
save_path = 'experiments_original_crossroads/'
...
def main():
    ...
    if bool(config.train):
        ...
        test_folder = "../../Crossroads/test/frames/01"
